document.addEventListener("DOMContentLoaded", function () {
  ...
  const DeleteModalClose = document.querySelector(
    "div[data-team-element=\"delete-modal-close\"]"
  );
  const editMembers = document.querySelectorAll(
    "img[data-team-element=\"edit-member\"]"
  );
  const deleteMembers = document.querySelectorAll(
  ...
  if (DeleteModalClose)
    DeleteModalClose.addEventListener("click", () => modalClose("delete-modal"));
  editMembers.forEach(editMember => {
    editMember.addEventListener("click", () => openModal("another-modal"));
  });
  deleteMembers.forEach(deleteMember => {
    deleteMember.addEventListener("click", () => openModal("delete-modal"));
  });
});
